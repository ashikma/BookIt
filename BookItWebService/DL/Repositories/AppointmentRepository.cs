...
namespace BookItWebService
{
    public class AppointmentRepository : Repository, IRepository<Appointment>
    {
        ...
        public List<Appointment> GetAppointmentsByDate(string start, string finish)
        {
            
            List<Appointment> list = new List<Appointment>();
            string sql = $@"SELECT Apointments.ApointmentID,
                            Apointments.userID, Apointments.Date,
                            Apointments.Time, Apointments.Oclock,
                            Apointments.TreatmentID, Apointments.Cost,
                            Apointments.ManagerId
                            FROM Apointments
                            WHERE CDate([Date])>=CDate('{start}') And CDate([Date])<=CDate('{finish}')
                            ORDER BY Cdate([Apointments.Date])";
            using (IDataReader reader = this._dbContext.Read(sql))
            {
                while (reader.Read())
                {
